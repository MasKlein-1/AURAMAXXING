...
class StructureSpec:
    ...
    def from_dict(cls, d: dict, where: str) -> "StructureSpec":
        ...
        if bool(d.get("cell")) == bool(d.get("from_file")):
            raise ValueError(
                f"{where}: provide exactly one of 'cell' (blank box) or 'from_file'"
            )
        from_file = d.get("from_file")
        if from_file is not None and not Path(from_file).exists():
            raise FileNotFoundError(
                f"{where}.from_file: structure file not found: {from_file}"
            )
        return cls(
            cell=d.get("cell"),
            pbc=d.get("pbc"),
...
def _parse_mix_string(s: str, where: str) -> list:
    ...
    if "=" in s:  # 'A:B=x:y'
        ...
        if len(comp_list) != len(ratio_list):
            raise ValueError(
                f"{where}.mix: '{s}' has {len(comp_list)} components but "
    ...
    if "+" in s or "%" in s:  # 'x% A + y% B'
        ...
        for token in s.split("+"):
            ...
            if not m:
                raise ValueError(
                    f"{where}.mix: could not parse component {token!r} in {s!r}"
                )
            items.append((m.group(2).strip(), float(m.group(1))))
        return items
    raise ValueError(
        f"{where}.mix: unrecognised mix {s!r}; use 'A:B=x:y', 'x% A + y% B', "
...
def _parse_mix(mix, where: str) -> dict:
    ...
    if isinstance(mix, dict):
        ...
        raise ValueError(
            f"{where}.mix: expected a mapping or a string, got {type(mix).__name__}"
        )
    weights: dict = {}
...
def _parse_composition_block(d: dict, where: str) -> dict:
    ...
    forms = [
        k for k in ("formula", "ratio", "mix", "counts", "target_ratios") if k in d
    ]
    if len(forms) != 1:
        raise ValueError(
            f"{where}: provide exactly one of 'formula', 'ratio', 'mix', "
    ...
    expected_total = (
        None
        if form == "counts"
        else ("target_number_atoms" if form == "target_ratios" else "total_atoms")
    )
    stray_total = ({"total_atoms", "target_number_atoms"} - {expected_total}) & set(d)
    if stray_total:
        raise ValueError(
            f"{where}: composition form {form!r} does not accept {sorted(stray_total)}"
            + (
                f"; use {expected_total!r}"
                if expected_total
                else " (explicit counts need no total)"
            )
        )

    if form == "counts":  # explicit counts: no total needed
        counts = {
            str(k): int(v) for k, v in _as_dict(d["counts"], f"{where}.counts").items()
        }
    ...
    if form == "formula":
        weights = {
            el: float(c) for el, c in _formula_counts(d["formula"], where).items()
        }
    elif form in ("ratio", "target_ratios"):
        weights = {
            str(k): float(v) for k, v in _as_dict(d[form], f"{where}.{form}").items()
        }
    else:  # mix
...
def _finalize_composition(parsed: dict, oxidation: dict, where: str) -> CompositionSpec:
    ...
    if adjusted:
        print(
            f"[composition] adjusted to the nearest charge-neutral integer counts: "
            f"{requested} -> {counts} (net formal charge 0)"
        )
    return CompositionSpec(
        target_counts=counts, requested_counts=requested, adjusted=adjusted
    )
...
class LimitSideSpec:
    ...
    def from_dict(cls, d: dict, where: str) -> "LimitSideSpec":
        d = _as_dict(d, where)
        _check_keys(
            d, {"type", "z", "z_av", "offset", "alpha", "n_max", "m_max"}, where
        )
        kind = _check_choice(
            d.get("type", "flat"), {"flat", "fourier", "surface"}, f"{where}.type"
        )
        if kind == "flat" and d.get("z") is None and d.get("offset") is None:
            raise ValueError(
                f"{where}: a flat limit needs 'z' (or 'offset' above a loaded surface)"
            )
        if kind == "fourier":
        ...
        return cls(
            ...
            offset=d.get("offset"),
            alpha=d.get("alpha"),
            n_max=int(d.get("n_max", 6)),
            m_max=int(d.get("m_max", 6)),
        )
...
class LimitsSpec:
    ...
    def from_dict(cls, d: dict, where: str) -> "LimitsSpec":
        ...
        return cls(
            bottom=LimitSideSpec.from_dict(d["bottom"], f"{where}.bottom"),
...
class CalculatorSpec:
    ...
    def from_dict(cls, d: dict, where: str) -> "CalculatorSpec":
        ...
        if addons_raw is not None and not isinstance(addons_raw, list):
            raise ValueError(
                f"{where}.addons: expected a list of calculator specs, got {type(addons_raw).__name__}"
            )
        return cls(
            type=d["type"],
            options=opts,
            addons=[_as_dict(a, f"{where}.addons") for a in (addons_raw or [])],
        )
...
class CalculatorsSpec:
    ...
    def from_dict(cls, d: dict, where: str) -> "CalculatorsSpec":
        ...
        return cls(
            growth=CalculatorSpec.from_dict(d["growth"], f"{where}.growth"),
            saturation=(
                CalculatorSpec.from_dict(sat, f"{where}.saturation") if sat else None
            ),
        )
...
class GrowthSpec:
    ...
    num_samples: int = 100
    anneal: Optional[dict] = (
        None  # {T_ini, T_fin, steps, interval}; None -> grow defaults
    )
    ...
    def from_dict(cls, d: dict, where: str) -> "GrowthSpec":
        ...
        _check_keys(
            d,
            {"max_placement_attempts", "per_anchor_attempts", "num_samples", "anneal"},
            where,
        )
        anneal = d.get("anneal")
        if anneal is not None:
            _check_keys(
                _as_dict(anneal, f"{where}.anneal"),
                {"T_ini", "T_fin", "steps", "interval"},
                f"{where}.anneal",
            )
        return cls(
            max_placement_attempts=int(d.get("max_placement_attempts", 1000)),
...
def _parse_fragment(raw, where: str) -> tuple:
    ...
    if isinstance(raw, str):
        ...
        isinstance(raw, (list, tuple)) and raw and all(isinstance(e, str) for e in raw)
    ):
        frag = tuple(raw)
    else:
        raise ValueError(
            f"{where}: expected an element symbol or a non-empty list of symbols, "
            f"got {raw!r}"
        )
    return frag
...
def _validate_mode(mode: str, pos: tuple, neg: tuple, where: str) -> None:
    ...
    """
    if mode == "anion_cap" and (
        pos != DEFAULT_POS_FRAGMENT or neg == DEFAULT_NEG_FRAGMENT
    ):
        raise ValueError(
            f"{where}.mode 'anion_cap': set a non-default negative_fragment "
            f"(e.g. F) and leave positive_fragment at the default H."
        )
    if mode == "cation_cap" and (
        neg != DEFAULT_NEG_FRAGMENT or pos == DEFAULT_POS_FRAGMENT
    ):
        raise ValueError(
            f"{where}.mode 'cation_cap': set a non-default positive_fragment "
...
class SaturationSpec:
    ...
    def from_dict(cls, d: dict, where: str) -> "SaturationSpec":
        ...
        pos = _parse_fragment(
            d.get("positive_fragment", list(DEFAULT_POS_FRAGMENT)),
            f"{where}.positive_fragment",
        )
        neg = _parse_fragment(
            d.get("negative_fragment", list(DEFAULT_NEG_FRAGMENT)),
            f"{where}.negative_fragment",
        )
        mode = d.get("mode")
        if mode is not None:
            mode = _check_choice(
                mode, {"neutral", "anion_cap", "cation_cap"}, f"{where}.mode"
            )
...
def _validate_fragments(sat: SaturationSpec, oxidation: dict, where: str) -> None:
    ...
    for label, frag, want, default in (
        ...
        if frag != default and len(frag) != 1:
            raise ValueError(
                f"{where}.{label}: a non-default fragment must be a single element "
                f"(1-valent cap); got {list(frag)}."
            )
        charge = sum(oxidation[e] for e in frag)
        if charge != want:
            raise ValueError(
                f"{where}.{label}: fragment {list(frag)} has net formal charge "
                f"{charge:+d}, but saturation fragments must be 1-valent "
                f"({want:+d})."
            )


def _validate_cn_distr_oxidation(
    cn_distr: dict, oxidation: dict, cap_elements: set, where: str
) -> None:
...
class ChargeCorrectionSpec:
    ...
    def from_dict(cls, d: dict, where: str) -> "ChargeCorrectionSpec":
        d = _as_dict(d, where)
        _check_keys(
            d, {"enabled", "max_iterations", "num_samples", "move_alpha"}, where
        )
        return cls(
            enabled=bool(d.get("enabled", False)),
...
class StatisticsSpec:
    ...
    surface: SurfaceSpec = field(
        default_factory=SurfaceSpec
    )  # cap areal-density sub-metric
...
class DebugSpec:
    ...
    def from_dict(cls, d: dict, where: str) -> "DebugSpec":
        ...
        _check_keys(
            d,
            {"write_growth_dumps", "write_trajectories", "pre_saturation_dump"},
            where,
        )
        return cls(
...
class RuleSpec:
    ...
    def from_dict(cls, d: dict, where: str) -> "RuleSpec":
        ...
        if "type" not in d:
            raise ValueError(f"{where}: missing required key 'type'")
        kind = _check_choice(
            d["type"], {"avoid_motif", "min_distance"}, f"{where}.type"
        )
...
class RunSpec:
    ...
    def from_dict(cls, d: dict, where: str) -> "RunSpec":
        ...
        fmt = _check_choice(
            d.get("output_format", "vasp"), {"vasp", "xyz"}, f"{where}.output_format"
        )
        seeds = d.get("seeds", [0])
        ...
        if rough is not None and (not isinstance(rough, list) or not rough):
            raise ValueError(
                f"{where}.roughness: expected a non-empty list of floats or omit it"
            )
        return cls(
...
class DFTSpec:
    ...
    slurm: bool = (
        False  # also emit a ready-to-edit SLURM array submission script per package
    )
    ...
    def from_dict(cls, d: dict, where: str) -> "DFTSpec":
        ...
        if not isinstance(packages, list) or not packages:
            raise ValueError(
                f"{where}.packages: expected a non-empty list of package names"
            )
        for p in packages:
        ...
        _check_keys(
            cp2k,
            {"project", "basis_set_file", "potential_file", "input"},
            f"{where}.cp2k",
        )
        return cls(
            enabled=bool(d.get("enabled", False)),
...
def _parse_cut_offs(raw: dict, where: str) -> dict:
    ...
    for key, val in _as_dict(raw, where).items():
        ...
        if len(parts) != 2:
            raise ValueError(
                f"{where}: cut_off key {key!r} must be 'Element-Element' (e.g. 'Si-O')"
            )
        a, b = parts
        out[(a, b)] = float(val)
        out[(b, a)] = float(val)
...
def _validate_cutoff_overrides(
    overrides: dict, cfg: CoordinationConfig, where: str
) -> None:
...
def _build_coordination(
    d: Optional[dict], where: str, *, elements, oxidation, spectators=(), terminal_cn=()
) -> CoordinationConfig:
    ...
    if d is not None:
        _check_keys(
            d,
            {"max_cn", "min_cn", "cut_offs", "cn_distr", "oxidation", "distance"},
            where,
        )
    distance_knobs = (
        _parse_distance_knobs(d["distance"], f"{where}.distance")
        if d and "distance" in d
        else None
    )
    ...
    max_cn_over = {
        ...
        **(
            {
                str(k): int(v)
                for k, v in _as_dict(d["max_cn"], f"{where}.max_cn").items()
            }
            if d and "max_cn" in d
            else {}
        ),
    }
    min_cn_over = {
        ...
        **(
            {
                str(k): int(v)
                for k, v in _as_dict(d["min_cn"], f"{where}.min_cn").items()
            }
            if d and "min_cn" in d
            else {}
        ),
    }

    tables = build_element_tables(
        elements,
        spectator_elements=spectators,
    ...
    cfg = CoordinationConfig(
        max_cn=tables["max_cn"],
        min_cn=tables["min_cn"],
        cut_offs=tables["cut_offs"],
        sample_dist=tables["sample_dist"],
        d_min_max=tables["d_min_max"],
...
class RunConfig:
    ...
    charge_correction: ChargeCorrectionSpec = field(
        default_factory=ChargeCorrectionSpec
    )
    ...
    def from_dict(cls, d: dict) -> "RunConfig":
        ...
        coord_block = (
            _as_dict(d["coordination"], "coordination")
            if d.get("coordination") is not None
            else None
        )
        ...
        sat_block = (
            _as_dict(d["saturation"], "saturation")
            if d.get("saturation") is not None
            else None
        )
        ...
        cap_elems = (
            _saturation_fragment_elements(sat_block)
            - (comp_parsed["elements"] | {"O", "H"})
            if sat_enabled
            else set()
        )
        elements = sorted(
            comp_parsed["elements"]
            | {"O", "H"}
            | _coord_override_elements(coord_block)
            | cap_elems
        )
        ...
        spectators = _loaded_elements(_as_dict(d["structure"], "structure")) - set(
            elements
        )
        ...
        _validate_cn_distr_oxidation(
            coordination.cn_distr, oxidation, cap_block, "coordination.cn_distr"
        )
        if saturation.enabled:
            _validate_fragments(saturation, oxidation, "saturation")
        _check_bks_supported(
            calculators,
            set(composition.target_counts),
            saturation.enabled,
            "calculators",
        )
        ...
        return cls(
            ...
            charge_correction=ChargeCorrectionSpec.from_dict(
                d.get("charge_correction", {}), "charge_correction"
            ),
