...
from base.initialize import (
    initialize_structure_blank,
    initialize_structure_file,
)
...
from saturation.new_sat import (
...
def _compose_addons(interface, addons: list) -> None:
    ...
    for addon_spec_dict in addons:
        ...
        addon = _build_addon(
            addon_type, {k: v for k, v in addon_spec_dict.items() if k != "type"}
        )
...
def resolve_plan(cfg: RunConfig) -> list[dict]:
    ...
    if run.roughness is not None and cfg.limits.top.type != "fourier":
        raise ValueError(
            "run.roughness sweeps the top fourier surface, but limits.top.type "
            "is not 'fourier'"
        )
    ...
    for seed, rough in combos:
        ...
        if single:
            ...
            out_path = (
                out_root / f"seed{seed}_alpha{_alpha_label(alpha)}" / f"structure.{ext}"
            )
...
def _apply_limit_side(
    ...
    if side.type == "surface":
        ...
        alpha = (
            alpha_override
            if (is_for == "top" and alpha_override is not None)
            else side.alpha
        )
        make_limits_fourier(
            struct,
...
def _combo_seed(seed: int, alpha: Optional[float]) -> np.random.Generator:
    ...
    alpha_bits = (
        0
        if alpha is None
        else int(np.array([alpha], dtype=np.float64).view(np.uint64)[0])
    )
...
def _init_structure(cfg: RunConfig):
    ...
    if s.from_file is not None:
        return initialize_structure_file(
            s.from_file, ase_read_kwargs=s.ase_read_kwargs, config=cfg.coordination
        )
...
def _generate_one(
    ...
    _finalize(growth_calc, traj_name="dry_final_opt")

    # Dump pre-saturation structure if requested
    if dbg.pre_saturation_dump:
        pre_sat_path = (
            out_path.parent / f"pre_sat_structure.{_EXT[cfg.run.output_format]}"
        )
    ...
    if cfg.saturation.enabled:
        ...
        _finalize(sat_calc, traj_name="undr_coord_opt")
        if cfg.charge_correction.enabled:
            cc = cfg.charge_correction
            correct_charge(
                struct,
                max_iterations=cc.max_iterations,
                num_samples=cc.num_samples,
                move_alpha=cc.move_alpha,
            )
            _finalize(sat_calc, traj_name="correct_charge_opt")
        ...
        if (
            sat.negative_fragment != DEFAULT_NEG_FRAGMENT
            or sat.positive_fragment != DEFAULT_POS_FRAGMENT
        ):
            relabel_caps(
                struct,
                negative_fragment=sat.negative_fragment,
                positive_fragment=sat.positive_fragment,
            )
            _finalize(sat_calc, traj_name="caps_swapped_opt")
    ...
    if cfg.rules:
        ...
        modifier.optimize()
        _finalize(sat_calc, traj_name="rules_applied_opt")
...
def _run_entry(cfg: RunConfig, entry: dict, growth_calc, sat_calc) -> dict:
    ...
    try:
        struct = _generate_one(
            cfg, entry["seed"], entry["alpha"], out_path, growth_calc, sat_calc
        )
        ...
        print(
            f"[runner] FAILED seed={entry['seed']} alpha={entry['alpha']}: "
...
def run_from_config(
    source: Union[str, Path, dict, RunConfig],
    *,
    workers: int = 1,
    ...
    if done:
        print(
            f"[runner] resume: skipping {len(done)} already-written of {len(plan)} structures"
        )
    ...
    written = [
        Path(r["output_path"]) for r in records if r["status"] in ("ok", "skipped")
    ]
    ...
    if cfg.statistics.enabled and cfg.statistics.pooled:
        if num_shards is None:
            ...
            print(
                f"[runner] sharded run: skipping pooled statistics; run "
    ...
    if cfg.dft.enabled and cfg.dft.slurm:
        if num_shards is None:
            ...
            print(
                f"[runner] sharded run: skipping DFT SLURM script; run `--pool-only` over "
...
def _write_manifest(
    ...
    name = (
        "manifest.json"
        if num_shards is None
        else f"manifest.shard{shard}of{num_shards}.json"
    )
    path = out_root / name
    payload = {
...
def _write_stats(
    ...
    try:
        ...
        metrics = analyze_structure(
            struct, is_saturation=is_saturation, surface_opts=surface_opts
        )
...
def _pool_stats(cfg: RunConfig) -> None:
    ...
    try:
        ...
        if merged is not None:
            ...
            print(
                f"[runner] pooled statistics skipped: fewer than 2 metrics.json under "
...
def _write_dft_slurm(cfg: RunConfig) -> None:
    ...
    try:
        ...
        written = write_slurm_scripts(
            Path(cfg.run.output_dir), dft_dirs, cfg.dft.packages, cp2k_project=project
        )
        if written:
            print(
                f"[runner] wrote DFT SLURM array script(s): "
                f"{', '.join(p.name for p in written)}"
            )
    except Exception as exc:
        print(
            f"[runner] DFT SLURM script generation FAILED: {type(exc).__name__}: {exc}"
        )
